...
def compute_info_path(data_mat, dimension_max, dimension_tot, nbtrials):
    Nentropie = {}
    # Compute all pairs of entropy and mutual information
    dimension_max_temp = dimension_max + 0
    dimension_max = 2
    allsubsets = lambda n: list(chain(*[combinations(range(1, n), ni) for ni in range(dimension_max + 1)]))
    list_tuples = allsubsets(dimension_tot + 1)
    ...
    for tuple_var in list_tuples:
        matrix_temp = np.hstack([
            np.reshape(data_mat[:, var - 1], (data_mat.shape[0], 1))
    ...
    tuple_maxinfo = ()
    for x, y in Ninfomut.items():
        if len(x) == 2 and y > max_info_temp:
            max_info_temp = y
            tuple_maxinfo = x
                ...
                for k in range(1, len(tuple_maxinfo) + 1):
                    for subset in itertools.combinations(tuple_maxinfo, k):
                        if subset not in Nentropie:
                            ...
                            for x, y in probability.items():
                                Nentropie[subset] = Nentropie.get(subset, 0) + self._information(probability[x])
                        Ninfomut[tuple_maxinfo] = Ninfomut.get(tuple_maxinfo, 0) + \
                                                  ((-1) ** (len(subset) + 1)) * Nentropie[subset]

        max_info_next = -100000
        for a, b in Ninfomut.items():
            if len(a) == current_dim and b > max_info_next:
                max_info_next = b
                tuple_maxinfo = a
...
#         return (Nentropie)
    ...
    def _compute_entropy(self, probability):
        ...
        # Compute entropy without progress tracking
        for x in range(0, (2 ** self.dimension_max) - 1):
            ntuple = []
            orderInf = 0
            self._decode_all(x, self.dimension_max, orderInf, ntuple)
            tuple_code = tuple(ntuple1_input[ntuple[z] - 1] for z in range(len(ntuple)))

            # Calculate probabilities
            probability2 = {}
            for x_key, x_prob in probability.items():
                code_proba = ''.join(
                    x_key[ntuple[i] - 1] if i < len(ntuple) and ntuple[i] == w else '0'
                    for w in range(1, self.dimension_max + 1)
                )
                probability2[code_proba] = probability2.get(code_proba, 0) + x_prob

            # Calculate entropy
            Nentropie[tuple_code] = sum(self._information(p) for p in probability2.values())
...
###################################################################################
    ...
    def _compute_forward_entropies(self, data_matrix):
        ...
        # Compute entropy for each subset without progress tracking
        for tuple_var in list_tuples:
            # Create a sub-matrix of data input for all subsets of variables
            matrix_temp = np.column_stack(
                [data_matrix[:, var - 1] for var in tuple_var]
            )

            # Compute probability and entropy for each submatrix
            probability = (
                self._compute_deformed_probability(matrix_temp)
                if self.deformed_probability_mode
                else self._compute_probability(matrix_temp)
            )

            # Calculate entropy
            for x, y in probability.items():
                Nentropie[tuple_var] = Nentropie.get(tuple_var, 0) + self._information(y)
...
#########################################################################
    ...
    def total_correlation_simplicial_lanscape(self, Nentropie):
        """
        Computes all total correlations or integrated information or free energy
        ...
        for x, y in Nentropie.items():
            sum_marginals = sum(Nentropie[(var,)] for var in x)
            total_corr = sum_marginals - y
            Ntotal_correlation[x] = total_corr
            list_tot_correlation[len(x)].append(total_corr)

            maxima_tot = max(maxima_tot, total_corr)
            minima_tot = min(minima_tot, total_corr)

        # Return the computed correlations without plotting
